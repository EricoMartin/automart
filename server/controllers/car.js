...
const CarAds = {
  async createAd(req, res) {
      ...
      let { manufacturer, model, price, state, status, year, body_type, owner, img } = req.body;
  ...
  async getCarByProp(req, res) {
    ...
    switch (param.toLowerCase()) {
      case 'manufacturer':
        ...
        //return cardata.rows.length < 1 ? res.status(404).json({status: 404, message: 'There are no cars for the selected manufacturer'}) : res.status(200).json({status: 200, data: carsdata.rows}); 
        //console.log(cardata);
        break;
        case 'model':
  ...
  async deleteCar(req, res) {
    ...
    try{
      ...
      const {rows} =  await Car.findCarAd(parseInt(req.params.car_id));
      if (rows.length < 1) {
        return res.status(404).json({
          status: 404,
      ...
      const deleteAd = await Car.deleteCar(parseInt(req.params.car_id));
      if (!deleteAd) {
        return res.status(404).json({
          status: 404,
