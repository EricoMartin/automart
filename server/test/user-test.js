...
import {
  createTables, users_seed,
  cars_seed, flags_seed, orders_seed,
} from '../migration/createTable';
...
describe(' User Endpoint Test', () => {
  ...
  describe('user can sign up', () => {
    it('should return 400 error if user is already signed up', (done) => {
      chai.request(app).post('/api/v1/auth/signup').set('authorization', token).send(myUser)
        .then((res) => {
    ...
    it('should return 400 error if user does not fill all required fields', (done) => {
      chai.request(app).post('/api/v1/auth/signup').set('authorization', token).send(ourUser)
        .then((res) => {
          expect(res.status).to.eq(400);
          expect(res.type).to.be.equal('application/json');
          expect(res.body).to.be.an('object');
    ...
    it('should return error if password is less than 8 characters', (done) => {
      chai.request(app).post('/api/v1/auth/signup').set('authorization', token).send(badPassUser)
        .then((res) => {
          expect(res.status).to.eq(400);
          expect(res.type).to.be.equal('application/json');
          expect(res.body).to.be.an('object');
    ...
    it('should return error if last_name or first_name is more than 30 characters', (done) => {
      chai.request(app).post('/api/v1/auth/signup').set('authorization', token).send(anUser)
        .then((res) => {
          expect(res.status).to.eq(400);
          expect(res.type).to.be.equal('application/json');
          expect(res.body).to.be.an('object');
    ...
    it('should return error if email is more than 30 characters', (done) => {
      chai.request(app).post('/api/v1/auth/signup').set('authorization', token).send(badEmailUser)
        .then((res) => {
          expect(res.status).to.eq(400);
          expect(res.type).to.be.equal('application/json');
          expect(res.body).to.be.an('object');
    ...
    it('should return error if password is not deeply equal to confirmed_password', (done) => {
      chai.request(app).post('/api/v1/auth/signup').set('authorization', token).send(badManUser)
        .then((res) => {
          expect(res.status).to.eq(400);
          expect(res.type).to.be.equal('application/json');
          expect(res.body).to.be.an('object');
