...
describe('Test car AD endpoint', function () {
  describe('Cars', function () {
    ...
    it('should create a new car ad', function () {
      _chai["default"].request(_app["default"]).post('/api/v1/car').set('Authorization', token).set('Content-Type', 'application/x-www-form-urlencoded').field('manufacturer', carAd.manufacturer).field('model', carAd.model).field('price', carAd.price).field('state', carAd.state).field('status', carAd.status).field('year', carAd.year).field('body_type', carAd.body_type).field('owner', 1).field('img', carAd.img).then(function (res) {
